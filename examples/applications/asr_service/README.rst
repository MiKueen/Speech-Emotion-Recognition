...
4) From `<nemo_git_root>/examples/applications/asr_service` folder do: `export FLASK_APP=asr_service.py` and start service: `flask run --host=0.0.0.0 --port=6006`
...
Also you must install Baidu's CTC decoder and KenLM. To do so (with KenLM built on LibriSpeech dataset) do:
